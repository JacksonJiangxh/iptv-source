# IPTV 播放列表

> 最后更新：2026-06-02 10:44:21 (北京时间)

## 📊 统计概览

| 项目 | 数量 |
|------|------|
| 总频道数 | **16702** |
| 已知别名 | 1863 |
| 分类数量 | 654 |
| GitHub 来源 | 39398 |
| 失败源数 | 397805 |

## 📺 频道分类 TOP 15

1. **未分类** - 1389 个频道
2. **GETROK** - 1148 个频道
3. **麻豆传媒** - 600 个频道
4. **HHH_798798** - 533 个频道
5. **🎎｜VIP直播资源3_0721** - 478 个频道
6. **Albania** - 472 个频道
7. **🎎｜VIP直播测试_0721** - 459 个频道
8. **卫视频道** - 361 个频道
9. **美食** - 357 个频道
10. **Turkey** - 353 个频道
11. **🔞国产传媒_8585** - 334 个频道
12. **传媒壹一_00** - 322 个频道
13. **纪录片频道** - 318 个频道
14. **央视频道** - 293 个频道
15. **Sport** - 292 个频道

## 📥 下载地址

- [output.m3u](output.m3u) - M3U 格式
- [output.txt](output.txt) - TXT 格式

## 📝 报告文件

- [new-aliasdemo.txt](new-aliasdemo.txt) - 详细分类报告
- [failed_sources.log](failed_sources.log) - 失败源日志
- [github_sources.log](github_sources.log) - GitHub 来源日志

## ⚠️ 需要人工处理的分类 (TOP 30)

以下分类未在 demo.txt 中定义，建议添加：

1. `$adultiptv`
2. `$mycamTV`
3. `$other adult iptv`
4. `$redtraffic adult iptv`
5. `-`
6. `168需要科学［美］节`
7. `336个地区`
8. `4GTV豪華套餐`
9. `4K频道`
10. `74.91.26.218:82`
11. `ABD`
12. `AEC`
13. `AKTV`
14. `ANIME Radio`
15. `ASIAN Radio`
16. `Ac25`
17. `Adult`
18. `Adventure`
19. `Afghan`
20. `Albania`
21. `Anak-Anak`
22. `Badminiton`
23. `Bangla`
24. `Business News`
25. `CCTV`
26. `CNR•广播`
27. `Comedy`
28. `Croatia`
29. `Cyprus`
30. `DEN'S TV`

> 共 581 个未知分类，完整列表见 [new-aliasdemo.txt](new-aliasdemo.txt)

## 📝 待添加到 alias.txt 的别名建议

以下频道名称建议添加别名映射：

- **TR: BLUMAX** (出现 40 次): TR: BLUMAX HD CINGILIKLER,TR: BLUMAX HD TURKU HD,TR: BLUMAX HD YESILCAM 5 4K
- 无主名 (出现 17 次): ,4K60PHLG-HEVC-EAC3测试,4K高清岳云鹏
- **浙江国际** (出现 8 次): 浙江国际
- **TVB星河** (出现 8 次): TVB星河,TVB星河频道,[MY]TVB星河
- **CGTN** (出现 7 次): CGTN
- **CCTV** (出现 7 次): CCTV-4K (1080p),CCTV-8K (1080p),CCTV4K
- **Arirang** (出现 7 次): Arirang,Arirang[1280x720],Arirang[640x360]
- **浙江钱江** (出现 6 次): 浙江钱江
- **浙江新闻** (出现 6 次): 浙江新闻
- **HBO** (出现 6 次): HBO,[MY]HBO,[TW]HBO HD
- **CNN** (出现 6 次): CNN,[MY]CNN
- **CNBC** (出现 6 次): CNBC,[MY]CNBC
- **HISTORY** (出现 6 次): HISTORY,HISTORY[1280x720],HISTORY频道
- **CHC动作电影** (出现 6 次): CHC动作电影
- **Hustler** (出现 6 次): Hustler,Hustler HD,Hustler HD orig
- **Anal** (出现 6 次): Anal,Anal 4K
- **FANT** (出现 6 次): FANTHD1,FANTHD4,FANTHD6
- **CGTN法语** (出现 5 次): CGTN法语
- **重温经典** (出现 5 次): 重温经典
- **Redlight** (出现 5 次): Redlight,Redlight HD,Redlight HD orig

> 共 160 条建议，完整列表见 [new-aliasdemo.txt](new-aliasdemo.txt)
