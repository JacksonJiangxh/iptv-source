# IPTV 播放列表

> 最后更新：2026-04-28 10:05:16 (北京时间)

## 📊 统计概览

| 项目 | 数量 |
|------|------|
| 总频道数 | **17068** |
| 已知别名 | 1863 |
| 分类数量 | 618 |
| GitHub 来源 | 20664 |
| 失败源数 | 334322 |

## 📺 频道分类 TOP 15

1. **未分类** - 1897 个频道
2. **GETROK** - 1230 个频道
3. **麻豆传媒** - 637 个频道
4. **HHH_798798** - 580 个频道
5. **🎎｜VIP直播测试_0721** - 485 个频道
6. **🎎｜VIP直播资源3_0721** - 478 个频道
7. **Albania** - 472 个频道
8. **Turkey** - 353 个频道
9. **纪录片频道** - 348 个频道
10. **🔞国产传媒_8585** - 341 个频道
11. **传媒壹一_00** - 340 个频道
12. **Sport** - 292 个频道
13. **VIP私密精选_00** - 266 个频道
14. **卫视频道** - 251 个频道
15. **EX-Yu** - 242 个频道

## 📥 下载地址

- [output.m3u](output.m3u) - M3U 格式
- [output.txt](output.txt) - TXT 格式

## 📝 报告文件

- [new-aliasdemo.txt](new-aliasdemo.txt) - 详细分类报告
- [failed_sources.log](failed_sources.log) - 失败源日志
- [github_sources.log](github_sources.log) - GitHub 来源日志

## ⚠️ 需要人工处理的分类 (TOP 30)

以下分类未在 demo.txt 中定义，建议添加：

1. `$adultiptv`
2. `$mycamTV`
3. `$other adult iptv`
4. `$redtraffic adult iptv`
5. `-`
6. `168需要科学［美］节`
7. `18+频道`
8. `336个地区`
9. `4GTV豪華套餐`
10. `4K`
11. `4K频道`
12. `74.91.26.218:82`
13. `ABD`
14. `AEC`
15. `AKTV`
16. `Ac25`
17. `Adult`
18. `Afghan`
19. `Albania`
20. `Anak-Anak`
21. `Badminiton`
22. `Bangla`
23. `Business News`
24. `CCTV`
25. `CNR•广播`
26. `Comedy`
27. `Croatia`
28. `Cyprus`
29. `DEN'S TV`
30. `DIY`

> 共 554 个未知分类，完整列表见 [new-aliasdemo.txt](new-aliasdemo.txt)

## 📝 待添加到 alias.txt 的别名建议

以下频道名称建议添加别名映射：

- **TR: BLUMAX** (出现 40 次): TR: BLUMAX HD ILYAS SALMAN,TR: BLUMAX HD KEMAL SUNAL 1 HD,TR: BLUMAX HD MASALLAR 2 HD
- 无主名 (出现 16 次): ,4K60PSDR-H264-AAC测试,4K高清岳云鹏
- **TVB星河** (出现 10 次): TVB星河,TVB星河频道,[MY]TVB星河
- **CCTV** (出现 9 次): CCTV-8k,CCTV4K,CCTV8K
- **HBO** (出现 7 次): HBO,[MY]HBO,[TW]HBO HD
- **Hustler** (出现 7 次): Hustler,Hustler HD,Hustler HD orig
- **7** (出现 7 次): 7
- **浙江新闻** (出现 7 次): 浙江新闻
- **浙江国际** (出现 7 次): 浙江国际
- **CGTN纪录** (出现 6 次): CGTN纪录
- **CGTN** (出现 6 次): CGTN
- **重温经典** (出现 6 次): 重温经典
- **CHC动作电影** (出现 6 次): CHC动作电影
- **Arirang** (出现 6 次): Arirang,Arirang[1280x720],Arirang[640x360]
- **Eroxxx** (出现 6 次): Eroxxx,Eroxxx HD
- **14** (出现 6 次): 14,14[1280x720]
- **HISTORY** (出现 6 次): HISTORY,HISTORY[1280x720],HISTORY频道
- **浙江钱江** (出现 6 次): 浙江钱江,浙江钱江频道[1920*1080]
- **Redlight** (出现 6 次): Redlight,Redlight HD,Redlight HD orig
- **4** (出现 6 次): 4

> 共 188 条建议，完整列表见 [new-aliasdemo.txt](new-aliasdemo.txt)
