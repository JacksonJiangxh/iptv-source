# IPTV 播放列表

> 最后更新：2026-06-09 10:12:22 (北京时间)

## 📊 统计概览

| 项目 | 数量 |
|------|------|
| 总频道数 | **15244** |
| 已知别名 | 1863 |
| 分类数量 | 665 |
| GitHub 来源 | 43198 |
| 失败源数 | 410250 |

## 📺 频道分类 TOP 15

1. **未分类** - 1360 个频道
2. **麻豆传媒** - 596 个频道
3. **HHH_798798** - 532 个频道
4. **🎎｜VIP直播资源3_0721** - 478 个频道
5. **Albania** - 472 个频道
6. **🎎｜VIP直播测试_0721** - 453 个频道
7. **美食** - 357 个频道
8. **Turkey** - 353 个频道
9. **🔞国产传媒_8585** - 334 个频道
10. **传媒壹一_00** - 321 个频道
11. **纪录片频道** - 316 个频道
12. **卫视频道** - 304 个频道
13. **Sport** - 292 个频道
14. **央视频道** - 256 个频道
15. **EX-Yu** - 242 个频道

## 📥 下载地址

- [output.m3u](output.m3u) - M3U 格式
- [output.txt](output.txt) - TXT 格式

## 📝 报告文件

- [new-aliasdemo.txt](new-aliasdemo.txt) - 详细分类报告
- [failed_sources.log](failed_sources.log) - 失败源日志
- [github_sources.log](github_sources.log) - GitHub 来源日志

## ⚠️ 需要人工处理的分类 (TOP 30)

以下分类未在 demo.txt 中定义，建议添加：

1. `$adultiptv`
2. `$mycamTV`
3. `$other adult iptv`
4. `$redtraffic adult iptv`
5. `-`
6. `168需要科学［美］节`
7. `336个地区`
8. `4GTV豪華套餐`
9. `4K频道`
10. `74.91.26.218:82`
11. `AEC`
12. `AKTV`
13. `ANIME Radio`
14. `ASIAN Radio`
15. `Ac25`
16. `Adult`
17. `Adventure`
18. `Afghan`
19. `Albania`
20. `Aleatórios`
21. `Anak-Anak`
22. `BD`
23. `Badminiton`
24. `Bangla`
25. `Business News`
26. `CCTV`
27. `CNR•广播`
28. `Comedy`
29. `Croatia`
30. `Cyprus`

> 共 595 个未知分类，完整列表见 [new-aliasdemo.txt](new-aliasdemo.txt)

## 📝 待添加到 alias.txt 的别名建议

以下频道名称建议添加别名映射：

- **TR: BLUMAX** (出现 40 次): TR: BLUMAX  HD YESILCAM 3  HD,TR: BLUMAX HD KADIR INANIR HD,TR: BLUMAX HD YESILCAM 6 HD
- 无主名 (出现 17 次): ,4K60PHLG-HEVC-EAC3测试,4K高清岳云鹏
- **TVB星河** (出现 8 次): TVB星河,TVB星河频道,[MY]TVB星河
- **CGTN** (出现 7 次): CGTN
- **CCTV** (出现 7 次): CCTV-4K (1080p),CCTV-8K (1080p),CCTV4K
- **Arirang** (出现 7 次): Arirang,Arirang[1280x720],Arirang[640x360]
- **浙江新闻** (出现 6 次): 浙江新闻
- **HBO** (出现 6 次): HBO,[MY]HBO,[TW]HBO HD
- **重温经典** (出现 6 次): 重温经典
- **浙江国际** (出现 6 次): 浙江国际
- **CNN** (出现 6 次): CNN,[MY]CNN
- **CHC动作电影** (出现 6 次): CHC动作电影
- **FANT** (出现 6 次): FANTHD1,FANTHD4,FANTHD5
- **浙江钱江** (出现 5 次): 浙江钱江
- **CGTN法语** (出现 5 次): CGTN法语
- **CNBC** (出现 5 次): CNBC,[MY]CNBC
- **Blonde** (出现 5 次): Blonde
- **Anal** (出现 5 次): Anal
- **CGTN纪录** (出现 4 次): CGTN纪录
- **Gay** (出现 4 次): Gay

> 共 132 条建议，完整列表见 [new-aliasdemo.txt](new-aliasdemo.txt)
